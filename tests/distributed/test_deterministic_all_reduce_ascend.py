...
"""Cross-TP deterministic all-reduce on Ascend NPUs.

Same contract as the CUDA test: eight ranks reduce with nested prefixes of the
same balanced tree for TP sizes 1/2/4/8; the result must be the canonical
8-leaf tree sum, bitwise identical across ranks, repeatable, and safe when
``out`` aliases ``input``.
"""
...
def test_deterministic_all_reduce_cross_tp_ascend() -> None:
    mp.spawn(
        _worker,
